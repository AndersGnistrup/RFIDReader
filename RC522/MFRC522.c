...
{
    uint8_t cmd[2] = { (addr << 1) & 0x7E, val };

    HAL_GPIO_WritePin(RC522_CS_GPIO_Port, RC522_CS_Pin, GPIO_PIN_RESET);
    HAL_SPI_Transmit(hspi, cmd, 2, 500);
...
{
    uint8_t cmd[2] = { ((addr << 1) & 0x7E) | 0x80, 0 };
    uint8_t rxData[2];

    HAL_GPIO_WritePin(RC522_CS_GPIO_Port, RC522_CS_Pin, GPIO_PIN_RESET);
    HAL_SPI_TransmitReceive(hspi, cmd, rxData, 2, 500);
    HAL_GPIO_WritePin(RC522_CS_GPIO_Port, RC522_CS_Pin, GPIO_PIN_SET);
 ...
 * Return value: None
 */
void MFRC522_Init(SPI_HandleTypeDef *_hspi)
{
    hspi = _hspi;
    HAL_GPIO_WritePin(RC522_CS_GPIO_Port, RC522_CS_Pin, GPIO_PIN_SET);
